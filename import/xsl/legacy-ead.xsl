...
<xsl:with-param name="TextToReplace" select="normalize-space(string(./eadheader/filedesc/titlestmt/titleproper))"/>
</xsl:call-template>
</field>
				...
				<xsl:variable name="htmlfilename"  select="concat(substring($xmlfilename, 1, string-length($xmlfilename) - 4), '.html')" />
				<field name="localfindingaidurl_str_mv">
					<xsl:value-of select="$htmlroot"/><xsl:value-of select="$htmlfilename"/>
				</field>				
